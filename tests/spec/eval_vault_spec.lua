...
describe("vault.evaluate — live", function()
	it("counts unlocked slots in the track (progress ≥ threshold)", function()
		local ns = harness()
		...
		assert.equal(2, r.progress); assert.equal(2, r.max)
	end)

	it("not enough unlocked → not done with unlocked/slots progress", function()
		local ns = harness()
	...
	it("ilvl floor excludes unlocked-but-low slots", function()
		...
		local r = ev(ns).evaluate({ track = "raid", slots = 2, ilvl = 600 })
		assert.is_false(r.done)
		assert.equal(1, r.progress)   -- only the 620 slot qualifies
	end)
...
describe("vault.evaluate — substrate branch", function()
	it("counts from the stored vault section", function()
		...
		local r = ev(ns).evaluate({ track = "raid", slots = 2 }, KEY)
		assert.is_true(r.done); assert.equal(2, r.progress)
	end)
